...
// ہادی بھائی، یہ آپ کا نیا لائیو بیک اینڈ یو آر ایل ہے
const API_BASE = "https://data-4ci5.vercel.app";

interface Lead {
  id?: number;
  company: string;   // بیک اینڈ کے مطابق 'company_name' کو 'company' کر دیا
  email: string;
  status: string;    // 'email_status' کو 'status' کر دیا
  city?: string;
  website?: string;
  name?: string;
  role?: string;
...
export default function Dashboard() {
  ...
  // 1. لیڈز لانے کا فنکشن
  const fetchLeads = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/leads`);
      if (!res.ok) throw new Error("Backend not responding");
      const data = await res.json();
      // بیک اینڈ سے آنے والا ڈیٹا 'data' کی کی (key) میں ہوتا ہے
      setLeads(data.data || []);
    } catch (error) {
  ...
  useEffect(() => {
    fetchLeads();
    // ہر 10 سیکنڈ بعد آٹو ریفریش
    const interval = setInterval(fetchLeads, 10000);
    return () => clearInterval(interval);
  }, []);

  // 2. سرچ شروع کرنے کا فنکشن
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.job_id) {
        setJobStatus(`Search started! Job ID: ${data.job_id.slice(0,8)}...`);
      }
  ...
  const renderStatusBadge = (status: string) => {
    ...
    if (s === "verified" || s === "deliverable" || s === "ok") {
              ...
              {leads.length > 0 ? leads.map((lead, idx) => (
                <tr key={idx} className="hover:bg-white/[0.03] transition-colors group">
                  <td className="px-6 py-6">
                    <div className="font-bold text-white text-lg group-hover:text-indigo-300 transition-colors">{lead.company}</div>
                    <div className="flex items-center text-sm text-gray-500 gap-1.5 mt-1">
                      <Globe className="w-3.5 h-3.5" />
                      <span className="text-xs">{lead.city || "Global Market"}</span>
                    </div>
                  </td>
                  <td className="px-6 py-6">
                    <div className="flex items-center gap-2.5 mb-1.5">
                      <div className="p-1.5 rounded-lg bg-indigo-500/10 text-indigo-400"><User className="w-4 h-4" /></div>
                      <span className="font-semibold text-gray-200">{lead.name || "Business Owner"}</span>
                    </div>
                  ...
                  <td className="px-6 py-6 text-center">
                    <div className="inline-flex flex-col items-center">
                       <span className={`text-lg font-black ${(lead.lead_score || 0) > 70 ? "text-emerald-400" : "text-indigo-400"}`}>
                        {lead.lead_score || 0}%
                      </span>
                      <div className="w-12 h-1 bg-white/10 rounded-full mt-1 overflow-hidden">
                        <div className="h-full bg-indigo-500" style={{ width: `${lead.lead_score || 0}%` }}></div>
                      </div>
                    </div>
                  </td>
                </tr>
